...
export default function OtpVerify() {
  ...
  const handleSubmit = async (event) => {
    ...
    const res = await fetch(
      "http://65.0.52.105:8006/api/method/consultant.api.login",
      {
        method: "POST",
        headers: {
    ...
    if (data.message.success_key == 1) {
      await AsyncStorage.setItem(
        "token",
        JSON.stringify(data.message.api_key + ":" + data.message.api_secret)
      );
  ...
  return (
    <View>
      <Text style={otpVerifyStyles.otpSentText}>
      ...
      {countdown > 0 ? (
        <Text style={otpVerifyStyles.resendText}>
